...
static const int skybox_vertices_stride = 3;
...
Skybox::Skybox(Skybox &&from) noexcept : _texture(from._texture), _VAO(from._VAO), _VBO(from._VBO), _files(std::move(from._files)) {
  from._texture = 0;
  from._VAO = 0;
  from._VBO = 0;
}
