...
auto Image::from_file(const std::filesystem::path& path, std::optional<Channels> desired_channels, bool flip)
    ...
    unsigned char* data = stbi_load(path.c_str(), &width, &height, &number_of_channels, desired_channels_numeric);
    ...
    auto image = Image(width, height, static_cast<Channels>(*desired_channels || number_of_channels), data);
...
Image::Image(Image&& from) noexcept : width(from.width), height(from.height), channels(from.channels), data(from.data) {
    from.data = nullptr;
}
